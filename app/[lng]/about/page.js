...
export default async function About({ params: { lng } }) {
  ...
  const sectionTwoTitle = () => {
    return (<span className={`${styles['text-text-highlight']}`}>{t('our-values')}</span>)
  }
  ...
  return (
    <>
      ...
      <Section title={sectionOneTitle} subtitle={null} content={sectionOneContent} wrapperContainer={false}/>
      <Section title={sectionTwoTitle} subtitle={null} content={sectionTwoContent} wrapperContainer={true}/>
